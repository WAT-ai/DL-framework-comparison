...
def main():
  ...
  class TimingCallback(tf.keras.callbacks.Callback):
    ...
    def on_test_batch_end(self, batch, logs={}):
      ...
      self.difference_batch_interference_list.append(self.endtime -
                                                     self.starttime)
  ...
  # Fitting the Model...
  model.fit(ds_train, epochs=10, validation_data=ds_test, callbacks=[cb])

  total_training_time = cb.end_train_time - cb.start_train_time
  average_epoch_training_time = np.mean(cb.difference_list)
  average_batch_interference_time = np.mean(
    cb.difference_batch_interference_list) * 1000
  ...
  metrics = {
    ...
    average_batch_interference_time  # in milliseconds
  }
